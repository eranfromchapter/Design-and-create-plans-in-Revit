...
| `mep_types.json` | Phase 6 MEP op emission (`create_pipe.pipe_type`, conduit type, `place_device` families), sim catalog membership, plugin symbol lookup | **PLACEHOLDER — ask Eran** for pipe/conduit types, PipingSystemType names and device families from Chapter's template |
