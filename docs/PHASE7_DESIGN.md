...
| P7-09 | `products.json` carries 14 `_PLACEHOLDER` SKUs (declared fields only; `catalog_version 0.1.0-placeholder`; `requires_human_input: true`) so goldens/e2e run; the validator refuses any `_PLACEHOLDER` SKU unless `allow_placeholders`, which the gateway sets only from `config.autoApprove` (CI). Real SKUs are Eran's. | E |
...
| P7-17 | Bridge deps: `opencv-python-headless` exact-pinned, `numpy>=2,<3`, `httpx`, `jsonschema`; a dev-only path dependency on `layout-compiler` for the golden chain builders (**deviation D-1**; runtime deps unchanged). | E |
...
Ladders (stable snake_case codes, each a named test): render-views `unknown_project → commit0_not_done → commit1_not_done → envelope_in_flight → render_export_in_progress`; compose-render `aidm_bridge_unavailable → blob_store_unavailable → no_render_job → render_export_failed → render_export_in_progress → render_review_pending | render_already_composed → blob_missing → blob_not_png → brief_not_confirmed → (bridge) render_failure card + 422`; finish-selection `no_render_review → render_not_approved → render_review_stale → finish_already_done → finish_review_pending → 400 (zod) → 422 bridge error | finish_selection_blocked | param_not_allowlisted (no card)`; issue-finish `no_finish_review → finish_review_not_approved → finish_already_done → finish_review_failed → envelope_in_flight → finish_reissue_exhausted`.
...
- `render_review.content`: `{render_id, export_envelope_id, layout_snapshot: commit1|commit2, control_maps[{name, kind, canny_ref, lines_ref, preview_ref, stats}], renders[{name, provider, ref, status}], prompt, candidates, finish_tier, brief_version, review_items, catalog_version, source_blob_refs, counts}` — refs only, never base64.
- `finish_commit.content`: `{selection, ops, catalog_version, render_ref, render_review_id, render_id, finish_tier, brief_version, review_items, diagnostics, counts}`; `content_hash` binds the ops.
...
|---|---|
| Control-map golden (fixture PNG → deterministic edge outputs) | bridge `test_control_maps.py` (byte goldens, deterministic-twice, live-sim tolerance, RGBA composite pitfall, limits), `test_fixture_drift.py` |
