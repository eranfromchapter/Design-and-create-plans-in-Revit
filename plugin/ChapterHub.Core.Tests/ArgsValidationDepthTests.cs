...
public sealed class ArgsValidationDepthTests
{
    ...
    {
        var seed = Convert.FromHexString(string.Concat(Enumerable.Repeat("f00dfeed", 8)));
        var body = """
        ...
        {
            Payload = body,
            Sig = Ed25519.SignHex(body, seed),
        };

        var result = EnvelopeVerifier.Verify(
            envelope,
            Convert.FromHexString(Ed25519.PublicKeyHexFromSeed(seed)),
